# Problem Framing Prompts

Personal collection of problem framing, project planning, PRD, business analysis, and product thinking prompts.

---

## Problem Framing Prompts

**Identify root causes and levers:**

```
What underlying factors or root causes are contributing to this issue — and which ones are within our influence to address?
```

Use to go beyond surface symptoms and identify levers for change.

*Source: [Rob Boyett](https://github.com/robboyett/Becoming-an-AI-designer/blob/main/Prompt-Lib/Prompt_problem-framing.md)*

---

**Reframe for fresh solutions:**

```
How might we reframe this problem to unlock fresh or unconventional solutions?
```

Encourages shifting perspective, challenging assumptions, or zooming in/out.

*Source: [Rob Boyett](https://github.com/robboyett/Becoming-an-AI-designer/blob/main/Prompt-Lib/Prompt_problem-framing.md)*

---

**Define across stakeholders:**

```
How can we define this problem in a way that reflects the needs, constraints, and motivations of all key stakeholders?
```

Ensures alignment across business, users, and technical considerations.

*Source: [Rob Boyett](https://github.com/robboyett/Becoming-an-AI-designer/blob/main/Prompt-Lib/Prompt_problem-framing.md)*

---

**Map driving forces over time:**

```
What forces — internal or external — are driving this problem, and how do they interact over time?
```

Useful for identifying systemic pressures, feedback loops, or legacy influences.

*Source: [Rob Boyett](https://github.com/robboyett/Becoming-an-AI-designer/blob/main/Prompt-Lib/Prompt_problem-framing.md)*

---

**Zoom out to the larger system:**

```
If we looked at this issue as part of a larger system, how would that shift our understanding of the problem and its boundaries?
```

Invites systems thinking, mapping relationships and dependencies.

*Source: [Rob Boyett](https://github.com/robboyett/Becoming-an-AI-designer/blob/main/Prompt-Lib/Prompt_problem-framing.md)*

---

**Surface baked-in assumptions:**

```
What assumptions are baked into our current problem definition — and what happens if we challenge them?
```

Helps break out of default framing, especially when it's limiting options.

*Source: [Rob Boyett](https://github.com/robboyett/Becoming-an-AI-designer/blob/main/Prompt-Lib/Prompt_problem-framing.md)*

---

**Test for unintended consequences:**

```
What risks or unintended consequences might arise from framing the problem in this particular way?
```

Prompts critical reflection before moving too quickly into solutioning.

*Source: [Rob Boyett](https://github.com/robboyett/Becoming-an-AI-designer/blob/main/Prompt-Lib/Prompt_problem-framing.md)*

---

**Look for where the problem doesn't show up:**

```
In what contexts or for which users does this problem not appear? What can that tell us?
```

A lateral way to explore exceptions and edge cases that might reveal blind spots.

*Source: [Rob Boyett](https://github.com/robboyett/Becoming-an-AI-designer/blob/main/Prompt-Lib/Prompt_problem-framing.md)*

---

## Planning Prompts

**Start a planning session:**

```
I want to build [feature]. Let me explain what I'm thinking, then ask me any clarifying questions you might have.

[describe your feature in natural language]
```

**Force clarification:**

```
Before we proceed, what assumptions are you making? What questions should I answer first?
```

**Break into issues:**

```
Break this plan into atomic chunks. Each chunk should be:
- Single-scope (one thing, done completely)
- Independently shippable
- Sequenced with dependencies noted

Post each chunk as a GitHub Issue.
```

---

**Learn while building:**

```
My goal is to make something but also to learn. Please look out for opportunities that you can use to grow my understanding as someone who is not that technical.
```

Use at the start of a session when you want the assistant to teach as it works — flagging concepts, choices, and patterns along the way for a less-technical user.

---

